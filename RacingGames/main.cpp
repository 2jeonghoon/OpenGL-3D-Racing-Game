...
void renderRaceTrack(Model& model, Shader& shader, int i);
...
void loadOBJ(const std::string& objFilePath, const std::string& mtlFilePath);
...
glm::vec3 lightPos(-1.0f, 100.0f, -1.0f);
...
unsigned int car_diffuseMap;
unsigned int bridge_texture;
unsigned int coin_texture;
...
int main()
{
	...
	printf("before carselect\n");

	Shader carShader_1("shader/carselect.vs", "shader/carselect.fs");

	Model carModel_1(FileSystem::getPath("asset/models/obj/P911/Porsche_911_GT2.obj"));
	car_diffuseMap = loadTexture("asset/models/obj/P911/skin01/0000.BMP");
	bridge_texture = loadTexture("asset/models/obj/road/road.jpg");
	coin_texture = loadTexture("asset/models/obj/coin/coin-texture.jpg");

	printf("car_diffuseMap\n");
	printf("car_normalMap\n");

	unsigned int car_normalMap = loadTexture("asset/models/obj/P911/skin01/0000-a.BMP");

	// Model carModel_2(FileSystem::getPath("asset/models/obj/redcar/redcar.obj"));
	Model coinModel(FileSystem::getPath("asset/models/obj/coin/coin.obj"));

	printf("shader use\n");

	shader.use();
	shader.setInt("diffuseTexture", 0);
	/*shader.setInt("normalMap", car_normalMap);*/
	...
	printf("before car select init\n");

	printf("car shader program\n");
	...
	while (!glfwWindowShouldClose(window)) {
		if (gamestatus == 0) {
			...
			{
				case 0: {
					Drawcarselect1(bgtexture, bgshaderProgram, bgVAO, shader, carModel_1, window);
		...
		else if (gamestatus == 4)
		{
			...
			{
			case 0: {
				renderCarAndCamera(carModel_1, shader);
				break;
			}
			//case 1: {
			...
			glm::vec3 coinpos[15] = {
				glm::vec3(22.0f, 0.0f, -7.3f),
				glm::vec3(-23.0f, 0.0f, 124.0f),
				glm::vec3(83.0f, 0.0f, 83.0f),
				glm::vec3(96.0f, 0.0f, -105.0f),
				glm::vec3(-8.0f, 0.0f, -151.0f),
				glm::vec3(-89.0f, 0.0f, -115.0f),
				glm::vec3(-92.0f, 0.0f, 156.0f),
				glm::vec3(44.0f, 0.0f, 210.0f),
				glm::vec3(100.0f, 0.0f, 170.0f),
				glm::vec3(-12.0f, 0.0f, 174.0f),
				glm::vec3(-60.0f, 0.0f, 100.0f),
				glm::vec3(-60.0f, 0.0f, -103.0f),
				glm::vec3(44.0f, 0.0f, -119.0f),
				glm::vec3(-25.0f, 0.0f, -27.0f),
			...
			if (coinindex == 3)gamestatus = 5;

			for (int i = 0; i < 12; i++) {
				renderRaceTrack(roadModels[i], shader, i);
			}
...
void renderCar(Model& model, glm::mat4 modelMatrix, Shader& shader)
{
	...
	glBindTexture(GL_TEXTURE_2D, car_diffuseMap);
	glActiveTexture(GL_TEXTURE15);
	glBindTexture(GL_TEXTURE_2D, depthMap);
...
//	shader.setInt("diffuseTexture", 0);
//	glActiveTexture(GL_TEXTURE0);
//	glBindTexture(GL_TEXTURE_2D, stop_sign_texture);
//
//	model.Draw(shader);
//}
...
void renderCoin(Model& model, Shader& shader, glm::vec3 a)
{
	...
	glm::vec3 pitchAxis(1.0f, 0.0f, 0.0f);
	glm::vec3 rollAxis(0.0f, 0.0f, 1.0f);
	float angle = glm::radians(90.0f);
	modelMatrix = glm::rotate(modelMatrix, angle, pitchAxis);
	modelMatrix = glm::rotate(modelMatrix, 1 * (float)glfwGetTime(), rollAxis);
...
void renderRaceTrack(Model& model, Shader& shader, int i)
{
	...
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0f, -1.23f, -20.0f + i * 4.3f));
	shader.setMat4("model", modelMatrix);
	...
	shader.setInt("diffuseTexture", 0);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, bridge_texture);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key == GLFW_KEY_C && action == GLFW_PRESS) {
		ourCamera.setFixed(car.Front);
		string info = ourCamera.isFixed() ? "camera change" : "camera change";
	...
	if (key == GLFW_KEY_X && action == GLFW_PRESS) {
		...
		string info = isPolygonMode ? "polygon mode change" : "polygon mode change";
...
void loadOBJ(const std::string& objFilePath, const std::string& mtlFilePath) {
	...
	std::vector<Material> materials = loadMTL(mtlFilePath); // MTL 颇老 肺靛
		...
		if (keyword == "usemtl") {
			...
			for (const auto& material : materials) {
				if (material.name == materialName) {
					// 犁龙 沥焊俊 蝶扼 咆胶贸 殿阑 利侩且 荐 乐澜
					std::cout << "Applying material: " << materialName << std::endl;
					break;
				}
			}
		}
...
std::vector<Material> loadMTL(const std::string& mtlFilePath) {
	...
	while (std::getline(file, line)) {
		...
		// 促弗 犁龙 加己俊 措茄 贸府档 鞘夸俊 蝶扼 眠啊且 荐 乐嚼聪促.
	}
